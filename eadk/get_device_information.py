...
def parse_slot_info(slot_info_file):
  ...
  kernel_header_address = int.from_bytes(result[1], "little")
  userland_header_address = int.from_bytes(result[2], "little")
...
def parse_userland_header(userland_header_file):
  ...
  external_apps_flash_start = int.from_bytes(result[1], "little")
  external_apps_flash_end = int.from_bytes(result[2], "little")
  external_apps_ram_start = int.from_bytes(result[3], "little")
  external_apps_ram_end = int.from_bytes(result[4], "little")
...
def extract_device_info(linker_script_file_name):
  ...
  try:
    ...
    subprocess.check_output([
        ...
        "0",
        "-s",
        f"{hex(userland_header_address)}:40:force",
        "-U",
        "userland_header.bin",
    ])
  ...
  with open(linker_script_file_name, "w") as linker_script:
    ...
    linker_script.write(
        f"EXTERNAL_APPS_FLASH_START = {hex(external_apps_flash_start)}" +
        ";\n")
    linker_script.write(
        f"EXTERNAL_APPS_FLASH_END = {hex(external_apps_flash_end)}" + ";\n")
    linker_script.write(
        f"EXTERNAL_APPS_RAM_START = {hex(external_apps_ram_start)}" + ";\n")
    linker_script.write(
        f"EXTERNAL_APPS_RAM_END = {hex(external_apps_ram_end)}" + ";\n")
